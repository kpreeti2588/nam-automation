...
//	private By allCardExpPaymentSectionNew = By.xpath("(//div[contains(@class,'theme-field')]//strong[contains(text(),'EX')])/..//span[1]");
	...
	public double getBalanceDue() {
		...
		Dictionary.put("DUE_BALANCE",
				String.valueOf(Double.parseDouble(getText(activeInvoiceBalanceDue).replaceAll("[^0-9.]", ""))));
...
//		}
		...
		try {
			for (int i = 0; i < amountToCharge.size(); i++) {
				...
				type(amountToCharge.get(i), "Enter Partial Amt", Environment.get("currency") + amountGiven, false,
						By.xpath("(//XCUIElementTypeTextField)[" + (i + 1) + "]"));
	...
	public void typeCVV(String cvv1) throws Exception {
		type(allCVVSelectPaymentPopUp, "CVV", cvv1);
	}
